-- ─────────────────────────────────────────────────────────────────────────────
-- Migration v11 — Server-side authentication
--
...
--
-- NOTE ON NUMBERING
--   This migration was originally authored as v10 on the feat/server-side-auth
--   branch, while a DIFFERENT v10 (the customers UNIQUE(phone) rebuild) was
--   authored in parallel on backend-audit-fixes. Two different files claimed the
--   same version number. Auth was renumbered to v11 because the customers
--   rebuild is the more destructive of the two and is safest to apply first,
--   while the database is still small and before auth rows exist.
--
--   Apply strictly in order: v9 -> v10 -> v11.
--
-- This migration is purely additive (two CREATE TABLE IF NOT EXISTS, no ALTER,
-- no DROP), so it is safe to re-run and does not touch existing data.
--
-- Apply against BOTH the prod and web D1 databases, e.g.:
--   wrangler d1 execute <DB_NAME> --file=./schema-migrate-v11.sql
--   wrangler d1 execute <DB_NAME> --file=./schema-migrate-v11.sql --config wrangler-web.toml
-- ─────────────────────────────────────────────────────────────────────────────
