 ...
 * writes from a cashier (or unauthenticated) session — mirroring
 * canPushSettingKey for settings. Local IndexedDB writes are unaffected; only
 * the doomed cloud round-trip is suppressed.
 *
...
describe('cashier menu writes are gated off the cloud', () => {
  it('has a manager-only menu push gate driven by the session role', () => {
    expect(src).toContain('function canPushMenuWrite()');
    expect(src).toContain("getSessionRole() === 'manager'");
  });

  it('pushMenuImmediate returns early for a cashier instead of enqueueing', () => {
    const gate = src.match(
      /async function pushMenuImmediate[\s\S]{0,200}?if \(!canPushMenuWrite\(\)\) return;/,
    );
  ...
  it('bootstrapPushAll is gated too (a cashier must not bulk-push the menu)', () => {
    const gate = src.match(
      /async bootstrapPushAll[\s\S]{0,250}?if \(!canPushMenuWrite\(\)\) return 0;/,
    );
    expect(gate).not.toBeNull();
  });
});
