...
export function NewOrderModal({ isOpen, onClose, menuItems, onSubmit }: NewOrderModalProps) {
  ...
  // Single-currency system (EGP) — matches POSView / Payment. Never hardcode $.
        ...
        >
          ...
          <div className="px-4 sm:px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 shrink-0">
            <div>
              ...
              <p className="text-xs text-gray-500">{t('Select items and choose order type')}</p>
            </div>
          ...
          <div className="flex flex-col sm:flex-row flex-1 overflow-hidden">
            ...
            <div className="flex-1 flex flex-col overflow-hidden border-b sm:border-b-0 sm:border-r border-gray-100">
              ...
              <div className="flex-1 overflow-y-auto p-3 grid grid-cols-2 md:grid-cols-3 gap-2 content-start">
                ...
                {filtered.length === 0 && (
                  <div className="col-span-2 md:col-span-3 text-center py-10 text-gray-400 text-sm">{t('No items found')}</div>
                )}
              </div>
            </div>
            ...
            <div className="sm:w-64 flex flex-col shrink-0">
              ...
              <div className="px-4 py-3 border-b border-gray-100 shrink-0">
                <label className="block text-xs font-semibold text-gray-700 mb-1.5">{t('Order Type')} *</label>
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-2">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
                  {t('Order Summary')} {cartItems.length > 0 && `(${cartItems.length} ${t('items')})`}
                </p>
                {cartItems.length === 0 ? (
                  <p className="text-xs text-gray-400 text-center py-6">{t('No items added yet')}</p>
                ) : (
                  cartItems.map(item => (
              ...
              <div className="p-4 border-t border-gray-100 space-y-3 shrink-0">
                ...
                >
                  {isSubmitting ? t('Creating...') : t('Place Order')}
                </button>
              </div>
            </div>
          </div>
