...
export function DatabaseStatus() {
  ...
  const checkConnection = async () => {
    setStatus('checking');
    // Web build: verify the Cloudflare D1 / IndexedDB data path is reachable.
    try {
    ...
    switch (status) {
      case 'checking':
        return {
          ...
          description: language === 'ar' ? 'يتم الآن فحص الاتصال بقاعدة بيانات Cloudflare D1 السحابية' : 'Verifying Cloudflare D1 cloud database connection',
        };
      case 'connected':
        return {
          icon: Wifi,
          ...
          label: language === 'ar' ? 'قاعدة البيانات السحابية متصلة' : 'Cloudflare D1 Connected',
          description: language === 'ar' ? 'قاعدة بيانات Cloudflare D1 متصلة وتعمل بكفاءة تامة أونلاين' : 'Cloudflare D1 cloud database is connected and fully operational online',
        };
      case 'error':
        return {
          icon: WifiOff,
          ...
          label: language === 'ar' ? 'خطأ في الاتصال بالسحاب' : 'Cloud Connection Error',
          description: language === 'ar' ? 'تعذر الاتصال بـ Cloudflare D1. يرجى التحقق من إعدادات الـ Worker' : 'Failed to connect to Cloudflare D1. Please check Worker settings.',
        };
    }
  };
          ...
          <div className="flex-1 min-w-0 text-left">
            ...
            <div className="space-y-1 text-xs text-gray-500">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span>{language === 'ar' ? 'سحابي (Cloudflare D1)' : 'Cloudflare D1 Database'}</span>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                <span className="font-semibold text-gray-700">{language === 'ar' ? 'رابط الاتصال:' : 'Endpoint:'}</span>
                <span className="font-mono">Cloudflare D1</span>
              </div>
              {lastChecked && (
